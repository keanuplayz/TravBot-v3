...
export default new Command({
	...
	async run($: CommonLibrary): Promise<any>
	{
		...
		if(difference < cooldown)
			return $.channel.send(`This command requires a day to cooldown. You'll be able to activate this command ${howLong}.`);
		else
							...
							if(emoteID && emoteID in stats)
							{
								...
								while(continueReactionLoop)
								{
									...
									else
									{
										...
										{
											$.warn(`[Channel: ${channel.id}, Message: ${msg.id}] A reaction count of ${reaction.count} was expected but was given ${userReactions} user reactions and ${botReactions} bot reactions.`);
											warnings++;
										}
									}
								}
